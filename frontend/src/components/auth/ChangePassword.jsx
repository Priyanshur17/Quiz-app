...
const ChangePassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const { data } = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/user/reset-password/${id}/${token}`, { password, confirmPassword }, {
            headers: {
  ...
  return (
    <form method="POST" className="w-full px-6 py-8 bg-white dark:bg-gray-800 rounded-lg" onSubmit={handleSubmit} noValidate>
      ...
      <div className="flex justify-center">
            ...
            >
                {loading ? <ImSpinner2 size={22} className="mx-auto animate-spin dark:text-white" /> : 'Save'}
            </button>
      </div>
